...
Regular number morphology is highly salient and carries its own semantic force: the \mention{-s} of \mention{books} signals atomic plurality, reinforcing an individuated construal (this is the bidirectional inference at work). \mention{cattle}, lacking any number contrast, provides no such reinforcement, leaving its number uncommitted. One morphosyntactic reflex: English nominal modifiers are characteristically singular (\mention{dog catcher}, not \ungram{\mention{dogs catcher}}), but \mention{cattle} and \mention{police} appear freely in modifier position (\mention{cattle prod}, \mention{police car}) precisely because their number isn't committed. That said, \mention{-s} isn't the whole story. Irregular plurals (\mention{children}, \mention{geese}) achieve number contrast without it; mass-like plurals (\mention{oats}, \mention{suds}, \mention{news}) carry \mention{-s} without supporting the rest of the count cluster. Number morphology is one property among several, one that can reinforce the cluster but doesn't by itself determine it.

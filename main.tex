...
% Legend
\node[housesecondary, font=\small] at (10.5,3) {$\blacktriangle$};
\node[font=\small, anchor=west] at (10.7,3) {tight};
\node[fill=housequinary, minimum size=5pt, inner sep=0pt] at (10.5,2.4) {};
\node[font=\small, anchor=west] at (10.7,2.4) {moderate};
\node[fill=houseprimary, circle, minimum size=5pt, inner sep=0pt] at (10.5,1.8) {};
\node[font=\small, anchor=west] at (10.7,1.8) {loose};
...
The preregistered probe (DOI: 10.5281/zenodo.19340604) tested this prediction using two frontier LLMs (Claude Sonnet 4.6 and GPT-5.4) rating 128 critical items (16 VP lexicalizations $\times$ 2 nouns $\times$ 2 determinatives $\times$ 2 constructions) plus 48 graduated fillers on a 1--7 Likert scale. Five independent runs per model yielded 1{,}760 ratings. Filler calibration confirmed that both models used the scale informatively (good fillers $\approx$ 7, degraded $\approx$ 2, ungrammatical $=$ 1).

The hierarchy was confirmed by both models. Claude rated \term{three folks} at 4.95 vs.\ \term{three people} at 7.00 (a 2.05-point suppression of the tight property); GPT-5.4 showed a smaller but consistent gap (5.93 vs.\ 6.92). Claude additionally distinguished tight from moderate: \term{several folks} (6.00) was rated higher than \term{three folks} (4.95), matching the hierarchy's prediction that the moderate property is more acceptable. GPT-5.4 did not make this distinction (\term{three folks} $\approx$ \term{several folks} $\approx$ 5.93), suggesting it represents the \term{folks} degradation as uniform rather than precision-graded.

The construction-sensitivity prediction was not confirmed. Neither model showed an existential--agentive asymmetry for \term{three folks} or any other cell (interaction $\approx$ 0 for both models). Within-model variance was near zero: both models gave effectively deterministic ratings within conditions. This null result is interpretable in three ways: (a) the construction-sensitivity prediction is wrong (existentials don't foreground individuation more than agentives); (b) prompted Likert ratings are too coarse to capture the effect (surprisal-based measurement might be more sensitive); or (c) LLMs don't represent construction-level individuation demands the way human processors do. The probe cannot distinguish these possibilities. What it does show is that the noun $\times$ determinative hierarchy is recoverable from LLM representations even though the construction $\times$ noun interaction is not.

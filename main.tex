...
The \term{folks} case functions as a boundary test. Quasi-count nouns like \term{cattle} are too categorical to reveal fine-grained construction effects; fully count nouns like \term{people} are too stable. A noun like \term{folks}, whose low-cardinal behaviour is genuinely unstable, offers the sharpest probe of whether the hierarchy reflects precision-sensitive bidirectional inference rather than static lexical features.
...
Together, the corpus data, the descriptive pattern, and the \term{folks} boundary case provide complementary evidence. The hierarchy captures categorical dissociation (quasi-count nouns), selective suppression of tight properties (\term{folks}), and construction-specific avoidance (quantified existentials with \term{cattle}). The construction-sensitivity prediction for \term{three folks} remains the sharpest untested prediction the account makes.

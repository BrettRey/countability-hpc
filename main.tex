...
I argue that morphosyntactic \textsc{countability} (distinct from ontological discreteness and semantic \textsc{individuation}) is a homeostatic property cluster (HPC) sustained by bidirectional inference: count syntax cues individuated construals in comprehension; individuated construals license count syntax in production. Because count properties differ in the \textsc{precision} of individuation they demand, when individuation weakens, properties dissociate in a predictable hierarchy: tight properties (singular, \term{a}(\term{n}), low cardinals) fail before loose ones (\term{many}, high round numerals, plural agreement).

The account derives the quasi-count pattern documented in \textit{CGEL}, the ordered dissociation pattern in the noun $\times$ property matrix, the stability of \term{cattle}/\term{police} (functional anchoring via \term{cow}/\term{officer}), and the instability of boundary cases like \term{folks}. Cross-linguistic and diachronic predictions follow that are absent from alternative approaches.
\end{abstract}
...
\item \term{Furniture}: ontologically discrete (composed of distinct objects), but semantically construed as an unbounded superordinate category rather than enumerable items, so morphosyntactically mass.
...
\begin{tikzpicture}[
    axis/.style={thick},
    grid/.style={gray!20},
    bar/.style={fill=houseprimary!60, draw=houseprimary!80, line width=4pt, rounded corners=1pt},
...
\node[accept]   at (0.55, -0.35) {\cmark};
\node[accept]   at (1.65, -0.35) {\cmark};
\node[accept]   at (2.75, -0.35) {\cmark};
\node[accept]   at (3.85, -0.35) {\cmark};
\node[accept]   at (4.95, -0.35) {\cmark};
\node[accept]   at (6.05, -0.35) {\cmark};
\node[accept]   at (7.15, -0.35) {\cmark};

% Row 2: people (suppletive sg, otherwise full count)
\node[marginal] at (0.55, -1.05) {\qmark};
\node[accept]   at (1.65, -1.05) {\cmark};
\node[accept]   at (2.75, -1.05) {\cmark};
\node[accept]   at (3.85, -1.05) {\cmark};
\node[accept]   at (4.95, -1.05) {\cmark};
\node[accept]   at (6.05, -1.05) {\cmark};
\node[accept]   at (7.15, -1.05) {\cmark};

% Row 3: folk BrE (no sg, no a(n), but low cardinals ok)
\node[reject]   at (0.55, -1.75) {\xmark};
\node[reject]   at (1.65, -1.75) {\xmark};
\node[accept]   at (2.75, -1.75) {\cmark};
\node[accept]   at (3.85, -1.75) {\cmark};
\node[accept]   at (4.95, -1.75) {\cmark};
\node[accept]   at (6.05, -1.75) {\cmark};
\node[accept]   at (7.15, -1.75) {\cmark};

% Row 4: folks AmE (no sg, no a(n), marginal low cardinals)
\node[reject]   at (0.55, -2.45) {\xmark};
\node[reject]   at (1.65, -2.45) {\xmark};
\node[marginal] at (2.75, -2.45) {\qmark};
\node[accept]   at (3.85, -2.45) {\cmark};
\node[accept]   at (4.95, -2.45) {\cmark};
\node[accept]   at (6.05, -2.45) {\cmark};
\node[accept]   at (7.15, -2.45) {\cmark};

% Row 5: cattle (quasi-count)
\node[reject]   at (0.55, -3.15) {\xmark};
\node[reject]   at (1.65, -3.15) {\xmark};
\node[reject]   at (2.75, -3.15) {\xmark};
\node[marginal] at (3.85, -3.15) {\qmark};
\node[accept]   at (4.95, -3.15) {\cmark};
\node[accept]   at (6.05, -3.15) {\cmark};
\node[accept]   at (7.15, -3.15) {\cmark};

% Row 6: police (quasi-count)
\node[reject]   at (0.55, -3.85) {\xmark};
\node[reject]   at (1.65, -3.85) {\xmark};
\node[reject]   at (2.75, -3.85) {\xmark};
\node[marginal] at (3.85, -3.85) {\qmark};
\node[accept]   at (4.95, -3.85) {\cmark};
\node[accept]   at (6.05, -3.85) {\cmark};
\node[accept]   at (7.15, -3.85) {\cmark};

% Row 7: furniture (mass)
\node[reject]   at (0.55, -4.55) {\xmark};
\node[reject]   at (1.65, -4.55) {\xmark};
\node[reject]   at (2.75, -4.55) {\xmark};
\node[reject]   at (3.85, -4.55) {\xmark};
\node[reject]   at (4.95, -4.55) {\xmark};
\node[reject]   at (6.05, -4.55) {\xmark};
\node[reject]   at (7.15, -4.55) {\xmark};
...
\centering
\begin{tikzpicture}[
    bar/.style={fill=houseprimary!60, draw=houseprimary!80},
    label/.style={font=\small}
]
...
Pointwise mutual information (PMI) scores from COCA provide independent confirmation. For quasi-count nouns, \term{three} shows negative PMI (the combination is rarer than chance would predict given the individual frequencies of determinative and noun): \term{three cattle} $-0.24$, \term{three police} $-2.67$, \term{three livestock} $-2.01$, \term{three vermin} $-\infty$. But \term{many} shows \textit{positive} PMI for the same nouns: \term{many cattle} $+1.72$, \term{many clergy} $+2.28$, \term{many vermin} $+1.08$. The tight property repels quasi-count nouns; the loose property attracts them. Opposite MI signs for the same nouns confirm that the ordering isn't a frequency artefact: it reflects a genuine distributional asymmetry between what tight and loose properties select for.

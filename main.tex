...
\begin{abstract}
English count properties (singular--plural contrast, \term{a}(\term{n}), low numerals, \term{many/few}, distributive quantifiers, plural agreement) form an exceptionally tight cluster, and the clustering is projectible: knowing that a noun takes \term{many} lets you predict plural agreement; knowing it takes \term{a}(\term{n}) lets you predict low cardinals. But in quasi-count nouns (\term{cattle}, \term{police}, \term{clergy}) the cluster dissociates in a constrained order. No existing account explains either the projectibility or its limits.

I argue that morphosyntactic \textsc{countability} (distinct from ontological discreteness and semantic \textsc{individuation}) is a homeostatic property cluster (HPC): a set of properties that hang together because bidirectional inference couples them to a common semantic variable, individuation. Because count properties differ in the \textsc{precision} of individuation they demand, when individuation weakens, properties dissociate in a predictable hierarchy: tight properties (singular, \term{a}(\term{n}), low cardinals) fail before loose ones (\term{many}, high round numerals, plural agreement). The mechanism explains why the projection works, not just that the properties co-occur.

The account derives the quasi-count pattern documented in \textit{CGEL}, the ordered dissociation in the noun $\times$ property matrix, the stability of \term{cattle}/\term{police} (functional anchoring via \term{cow}/\term{officer}), and the instability of boundary cases like \term{folks}. Corpus evidence from COCA confirms that the ordering tracks semantic precision, not collocation frequency.
\end{abstract}
...
Individuation operates at two levels. Lexemes carry conventionalized construal profiles (default settings for how their referents are packaged, established through usage history and stored in lexical knowledge). These profiles make \term{cattle} quasi-count and \term{book} fully count as types. But speakers can override defaults in context: \term{three coffees} coerces a mass noun into a count frame. The bidirectional inference mechanism operates on both levels: conventionalized profiles constrain expectations; online construal can shift them. The hierarchy developed here concerns conventionalized profiles (the stable defaults that determine a noun's typical countability behaviour), not the contextual coercions that temporarily override them. Because those profiles are established by distributional history (frequency, collocational patterns, register) rather than read off any single morphosyntactic property, the account avoids circularity.
...
This framework provides two diagnostics for whether countability is a genuine HPC rather than a convenient label \citep{reynolds2025stack}. The \textit{projectibility test} asks whether learning a noun's count properties from partial evidence predicts held-out data. It does: if a noun takes \term{many}, one can reliably predict plural agreement; if it takes \term{a}(\term{n}), one can predict low cardinals. The implicational hierarchy developed in \S\ref{sec:hierarchy} is exactly this predictive structure: the pattern travels. The \textit{homeostasis test} asks whether we can name the stabilizers and whether they're perturbation-sensitive. We can identify at least these: bidirectional inference, entrenchment, functional anchoring, institutional norms. And they are: remove \term{datum} from active use and \term{data} drifts toward mass; if \term{officer} were to disappear, we'd predict \term{police} to drift toward full count or full mass rather than remaining stably quasi-count. A mere label (like the traditional category \enquote{adverb}) would be perturbation-inert: remove any one supposed stabilizer and nothing changes, because nothing holds it together causally. Countability passes both tests.

A residual question: is the count cluster genuinely homeostatic, or is it merely a set of indicators that co-vary because they share a common cause (individuation)? A parametric model with one latent variable and multiple threshold indicators would produce the same implicational structure without any feedback. The distinction matters. In a shared-cause model, perturbing one indicator doesn't affect the others; in a homeostatic system, it does. The evidence for feedback comes from acquisition and processing. Children who encounter a noun in one count frame extend to others: hearing \term{many} N generates expectations for \term{three} N, and the overgeneralization errors documented in \S\ref{sec:homeostasis} (\term{a money}, \term{a spooky furniture}) are exactly such extensions. This isn't passive co-observation of indicators that happen to share a cause; it's active inference from one property to another through the individuation variable. Similarly, violations of cluster coherence incur processing costs that reinforce the canonical pattern: \term{three cattle} feels wrong not because \term{three} and \term{cattle} are individually anomalous but because the combination violates an expectation generated by \term{cattle}'s other count properties. The feedback is real: one property's presence or absence causally affects expectations about the others, and those expectations shape acquisition and production. That is what distinguishes a homeostatic cluster from a set of correlated indicators.
...
\footnote{Bare singulars in these idiomatic contexts don't show full mass behaviour; they pattern with weakened individuation, so a tight-linkage property drops out while looser properties (plural agreement, \term{many}) remain available. This matches the hierarchy's prediction.}

Related diagnostics align with the same ordering. Demonstratives are tight: plural-only nouns pattern with \term{these/those} but reject \term{this/that} (\term{these police}, \ungram{\term{this police}}), while mass nouns reject both unless coerced. Pronominal anaphora and subject--verb agreement are loose: \term{the police ... they/*it} is grammatical even when tighter properties fail.
...
Recall \term{cattle} from \S\ref{sec:levels}: ontologically discrete, semantically aggregated. The hierarchy predicts it should accept loose properties (plural agreement, \term{many}) while rejecting tight ones (singular, \term{a}(\term{n}), low cardinals). The matrix in \S\ref{sec:evidence} confirms this.
...
\node[label, rotate=45, anchor=west] at (0.5, 0.5) {\textsc{sg/pl}};
...
COCA frequencies provide quantitative support for the hierarchy. Table~\ref{tab:coca} reports counts for \term{three}, \term{many}, and the tight diagnostic \term{how many} with four nouns: \term{people} (fully count), \term{folks} (boundary case), \term{police} (quasi-count), and \term{cattle} (quasi-count). Because raw string searches conflate head uses with other functions, the counts require interpretation.
...
\toprule
Noun & COCA freq. & \term{three} & \term{many} & \term{how many} & \term{three}:\term{many} & \term{three}/M & \term{how many}/M \\
\midrule
\term{people}  & 1{,}784{,}505 & 3{,}972 & 48{,}153 & 7{,}919 & 8.3\%  & 2{,}226 & 4{,}438 \\
\term{folks}   & 65{,}895    & 17    & 783    & 85     & 2.2\%  & 258    & 1{,}290 \\
\term{police}  & 220{,}858   & 20    & 74     & 11     & 27.0\% & 91     & 50 \\
\term{cattle}  & 14{,}376    & 7     & 43     & 8      & 16.3\% & 487    & 557 \\
\bottomrule
\end{tabular}
...
This asymmetry (\term{people} and \term{folks} don't function as modifiers; \term{police} and \term{cattle} do) is itself diagnostic. English nominal modifiers are characteristically singular (\term{dog catcher}, not \ungram{\term{dogs catcher}}). The fact that \term{police} and \term{cattle} appear freely in modifier position despite plural morphology indicates that their number specification is uncommitted, vague enough to satisfy a structural slot that ordinarily requires singular form. COCA collocate searches confirm this quantitatively: 23.5\% of all \term{cattle} tokens are followed immediately by another noun (\term{cattle} + [n*]), compared to 1.0\% for \term{people}, 0.9\% for \term{cows}, and 0.6\% for \term{folks}. The pattern extends to the other quasi-count nouns: \term{poultry} 41.3\%, \term{youth} 32.4\%, \term{livestock} 26.7\%, \term{clergy} 9.1\%, \term{vermin} 6.2\%. Every noun without overt plural marking modifies at 6\% or higher; every noun with plural \term{-s} or a suppletive plural is under 1\%. The \term{cattle}/\term{cows} comparison is the cleanest: same referents, same semantic domain, but \term{cows} (fully count, committed number) barely modifies, while \term{cattle} (quasi-count, uncommitted number) does so at 25 times the rate. \term{Folks}, despite its weakened individuation, has overt plural \term{-s} and patterns with the number-committed nouns for modifier rate. The modifier diagnostic thus tracks number morphology specifically, not countability in general, confirming its position among the tightest properties in the hierarchy.
...
\toprule
Determinative & COCA freq. & Precision & PMI \term{cattle} & PMI \term{folks} & PMI \term{people} \\
\midrule
\term{two}      & 1{,}141{,}704 & tight    & $-2.46$ & $-0.51$ & $+2.53$ \\
\term{three}    & 571{,}941     & tight    & $-0.24$ & $-1.16$ & $+1.95$ \\
\term{each}     & 545{,}542     & tight    & $-1.98$ & $-5.18$ & --- \\
\term{several}  & 251{,}581     & moderate & $-\infty$ & $+0.52$ & $+1.35$ \\
\term{various}  & 99{,}848      & moderate & $+0.47$ & $+1.52$ & $+1.59$ \\
\term{many}     & 904{,}039     & loose    & $+1.72$ & $+3.71$ & $+4.89$ \\
\term{numerous} & 35{,}176      & loose    & $+0.97$ & $+1.59$ & $+2.00$ \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize{Note: PMI = pointwise mutual information; positive = attracted (more frequent than chance), negative = repelled (less frequent than chance). Dashes indicate cells where the \enquote{persons} sense cannot be isolated (all \term{each people} tokens are the ethnic-group sense). \term{Two cattle} filtered to $\approx$3 head uses from 10 raw (7 modifier uses). \term{Every} omitted (all quasi-count tokens are modifier uses or the ethnic-group sense).}
\end{table}

The critical comparison is \term{two} vs.\ \term{numerous}. \term{Two} is 32 times more frequent than \term{numerous}, but \term{two cattle} is repelled ($\text{PMI} = -2.46$) while \term{numerous cattle} is attracted ($+0.97$). The same pattern holds for \term{folks}: \term{two folks} ($-0.51$) vs.\ \term{numerous folks} ($+1.59$). If frequency drove the hierarchy, the most frequent determinative should combine most freely; instead, the pattern tracks precision demand. Every tight determinative repels quasi-count nouns regardless of its frequency; every loose determinative attracts them regardless of its rarity. This confirms that the ordering reflects what each determinative semantically requires, not how often it occurs.
...
\node[font=\small\bfseries, above] at (3.25,3.5) {\term{cattle}};
...
\node[font=\small, rotate=90, anchor=south] at (-0.6,0) {PMI};
\node[font=\small, below] at (3.25,-3.8) {determinative frequency ($\log_{10}$)};
\node[font=\small\bfseries, above] at (3.25,3.5) {\term{folks}};
\draw[gray!40, dashed] (0,0) -- (6.3,0);
\draw (-0.1,-3) -- (0.1,-3); \node[font=\tiny, left] at (-0.15,-3) {$-3$};
\draw (-0.1,-2) -- (0.1,-2); \node[font=\tiny, left] at (-0.15,-2) {$-2$};
\draw (-0.1,-1) -- (0.1,-1); \node[font=\tiny, left] at (-0.15,-1) {$-1$};
...
% officers (sing.)
\node[label, anchor=east] at (-0.15,6.83) {\term{officers} \scriptsize{(sing.)}};
...
% chickens (sing.)
\node[label, anchor=east] at (-0.15,6.17) {\term{chickens} \scriptsize{(sing.)}};
...
% priests (sing.)
\node[label, anchor=east] at (-0.15,5.53) {\term{priests} \scriptsize{(sing.)}};
...
% people
\node[label, anchor=east] at (-0.15,4.88) {\term{people}};
...
% folks (boundary)
\node[label, anchor=east] at (-0.15,4.23) {\term{folks} \scriptsize{(boundary)}};
...
% clergy
\node[label, anchor=east] at (-0.15,3.58) {\term{clergy}};
...
% cattle
\node[label, anchor=east] at (-0.15,2.93) {\term{cattle}};
...
% youth
\node[label, anchor=east] at (-0.15,2.27) {\term{youth}};
...
% poultry
\node[label, anchor=east] at (-0.15,1.62) {\term{poultry}};
...
% livestock
\node[label, anchor=east] at (-0.15,0.98) {\term{livestock}};
...
% vermin
\node[label, anchor=east] at (-0.15,0.33) {\term{vermin}};
...
\node[label, anchor=west] at (3.2,7.75) {\term{three} N};
\node[dot many] at (5,7.75) {};
\node[label, anchor=west] at (5.2,7.75) {\term{many} N};

\end{tikzpicture}
\caption{Per-million frequency of \term{three} N (coral) and \term{many} N (blue) with 95\% Poisson confidence intervals, plotted on a $\log_{10}$ scale. Singulatives cluster at top right (both quantifiers well attested). Quasi-count nouns show the dissociation: \term{many} rates overlap with singulatives, but \term{three} rates plummet. Intervals for high-count nouns (\term{people}, \term{officers}) are too narrow to see at this scale; intervals for small counts (\term{three clergy}, \term{many poultry}, \term{many vermin}) are wide. The contrast is the point: some estimates are precise, others are not. Head uses only for \term{police} and \term{cattle}; \term{many} excludes \term{how many}.}
\label{fig:dotplot}
\end{figure}

\subsection{Existential construction probe}\label{sec:existential}

The hierarchy predicts that constructions foregrounding exact cardinality should be especially hostile to quasi-count nouns. Existential \term{there} constructions do this: \term{there are three people in the room} presupposes a precise count of discrete entities. If the tight/loose distinction reflects precision demands, existentials should suppress tight quantifiers with quasi-count nouns more than other frames do.

Table~\ref{tab:exist} reports COCA counts for \term{there [be] [det] [noun]} (existential frame) and \term{[det] [noun] [v*]} (verb-following frame) across three determinatives and three nouns.
...
\midrule
\term{three} & \term{people} & 128 & 1{,}356 & 1{,}484 & 8.6\% \\
 & \term{folks} & 2 & 1 & 3 & --- \\
 & \term{cattle} & 0 & 3 & 3 & --- \\
\midrule
\term{several} & \term{people} & 120 & 1{,}155 & 1{,}275 & 9.4\% \\
 & \term{folks} & 6 & 24 & 30 & 20.0\% \\
 & \term{cattle} & 0 & 0 & 0 & --- \\
\midrule
\term{many} & \term{people} & 1{,}046 & 29{,}449 & 30{,}495 & 3.4\% \\
 & \term{folks} & 18 & 433 & 451 & 4.0\% \\
 & \term{cattle} & 0 & 11 & 11 & 0\% \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize{Note: Dashes indicate cells too sparse ($N < 10$) for meaningful percentages. Verb-following counts for \term{cattle} are filtered for head uses (see \S\ref{sec:corpus}). \term{Cows} is included in the discussion for comparison: \term{there [be] three cows} = 1, \term{there [be] many cows} = 2.}
\end{table}

Three findings emerge.

First, the loose property shows no construction effect. \term{Many people} and \term{many folks} have similar existential shares (3.4\% vs.\ 4.0\%). The loose quantifier distributes evenly across constructions for both nouns, as the hierarchy predicts: if \term{many} tolerates low-precision individuation, the existential frame's higher precision demand shouldn't matter.

Second, the construction-sensitivity prediction for the tight property (\term{three folks}) can't be tested: only 3 tokens survive. The suppression operates at the \term{three} + \term{folks} level before any construction effect can be measured. This motivates an acceptability study (\S\ref{sec:folks}) where controlled stimuli can probe the gradient that corpus data are too sparse to reveal.

Third, \term{cattle} categorically avoids quantified existentials. Across \term{three}, \term{several}, and \term{many}, \term{there [be] [det] cattle} returns zero tokens. But bare \term{cattle} in existentials is grammatical: \term{there [be] cattle} returns 6 tokens (\term{there are cattle on his ranch}, \term{there are cattle loose between some of the estates}). The existential frame doesn't reject \term{cattle} itself; it rejects \textit{quantified} \term{cattle}. Quantifiers impose individuation demands that the existential frame amplifies; \term{cattle}'s aggregate semantics can't satisfy both simultaneously. This is evidence against a purely morphological explanation: if \term{cattle} were simply a defective plural, bare and quantified existentials should pattern alike. They don't.
...
\textit{CGEL} contrasts these with exceptional plural-only nouns like \term{troops} and \term{folk}, which accept low numerals despite lacking a simple morphological singular of the same lexeme: \term{three troops}, \term{several folk}, but \ungram{\term{a troop}} (in the sense of one soldier) or \ungram{\term{a folk}} in their ordinary senses \citep[p.~345]{huddleston2002}.\footnote{\term{Troop} as a collective unit exists (\term{a scout troop}), but \term{troops} meaning soldiers has no singular \term{a troop}. \term{A folk} survives in archaic registers. The key point is that these nouns permit low cardinals despite the morphological gap.} The grammar notes the distinction but offers no explanation for why some plural-only nouns permit low cardinals while others don't.

The dissociation hierarchy provides the explanation. The difference between \term{troops}/\term{folk} and \term{cattle}/\term{police} isn't morphological (both types lack singulars) but semantic: how accessible is individuation? \term{Troops} and \term{folk} provide access to discrete, enumerable individuals; \term{cattle} provides only partial access, enough for \term{many} (magnitude assessment) but not for \term{three} (exact enumeration). When individuation is weakened (when referents are construed as aggregates rather than discrete units), tight properties fail first. Loose properties remain because they tolerate low-precision magnitude assessment without requiring precise atomic boundaries.
...
Other quasi-count nouns show slight variations but respect the hierarchy. Table~\ref{tab:expansion} reports COCA frequencies for five additional quasi-count nouns. \term{Poultry} and \term{livestock} largely pattern like \term{cattle}: near-zero tokens with \term{three} and \term{several}, modest acceptance of \term{many}. \term{Vermin} is the most extreme case: zero tokens with \term{three} or \term{several}, and only 2 with \term{many}. \term{Clergy} is more permissive, accepting \term{many clergy} (26) and marginal \term{several clergy} (4), with \term{cleric} serving the singulative function. \term{Youth} patterns nearest to \term{folks}, accepting \term{many youth} (86), \term{several youth} (22), and even \term{three youth} (10), though some of the latter may reflect the fully count sense (\term{a youth} = a young person). None of these nouns reverses the implicational pattern: none accepts tight properties while rejecting loose ones.
...
\toprule
Noun & COCA freq. & \term{three} & \term{several} & \term{many} & \term{how many} \\
\midrule
\term{youth}     & 49{,}846 & 10 & 22 & 86 & 3 \\
\term{clergy}    & 5{,}879  & 3  & 4  & 26 & 1 \\
\term{livestock} & 6{,}982  & 1  & 1  & 13 & 0 \\
\term{poultry}   & 4{,}544  & 1  & 1  & 3  & 0 \\
\term{vermin}    & 1{,}041  & 0  & 0  & 2  & 0 \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize{Note: Raw counts; no modifier filtering applied (\term{poultry} and \term{livestock} function freely as modifiers, so head-use counts may be lower). Nouns ordered by acceptance of tight properties. Compare Table~\ref{tab:coca} for core nouns.}
\end{table}
...
The corpus data from \S\ref{sec:existential} confirm the suppression: \term{three folks} has only 3 total tokens in COCA (2 existential, 1 verb-following), too sparse to test construction-sensitivity. The suppression operates at the \term{three} + \term{folks} level before any construction effect can be measured. An acceptability study is needed to probe the gradient that corpus data can't reveal.

Because the critical \term{three folks} cell is too sparse to support a direct corpus estimate of construction-sensitivity, I supplement the corpus analysis with a preregistered LLM-based judgment probe. The corpus serves as the primary benchmark for the attested community profile of countability in these nouns; the model is used only to test whether the hierarchy's predicted extrapolation to sparse cells is recoverable under a fixed elicitation regime. On the present account, such outputs are not direct evidence of grammatical status itself. They are secondary evidence about detector-level responses to a boundary construction, to be interpreted against the corpus record rather than in place of it.%
...
The hierarchy predicts that if construction-sensitivity exists, it should be specific to tight properties. Existential constructions foreground exact cardinality (\term{There are three folks waiting in the lobby}); agentive constructions allow it to be backgrounded (\term{Three folks signed the complaint}). \term{Three folks} should show a stronger existential--agentive asymmetry than \term{three people}, and \term{several folks} should show little or none. A 2$\times$2$\times$2 design crossing noun (\term{people} vs.\ \term{folks}), determinative (\term{three} vs.\ \term{several}), and construction type (existential vs.\ agentive), with \term{cattle} as a floor, would test this directly.%
\footnote{For many speakers, colloquial \term{there's three folks} is more acceptable than \term{there are three folks}, likely reflecting register congruence and the lower precision demands of the invariant existential. The standard agreeing form \term{there are} maintains the high-precision condition. \term{There are three guys} (equally informal but fully count) is acceptable, so the \term{folks} degradation isn't purely register mismatch.}

A preregistered probe (DOI: 10.5281/zenodo.19340604) tested this prediction using two frontier LLMs (Claude Sonnet 4.6 and GPT-5.4). Both models confirmed the noun $\times$ determinative hierarchy: \term{three folks} was rated substantially lower than \term{three people}, matching the tight-property suppression observed in COCA. The construction-sensitivity prediction was not confirmed: neither model showed an existential--agentive asymmetry. Full design, cell means, model comparison, and interpretation are in Appendix~\ref{app:probe}; stimuli, data, and analysis code are at \url{https://github.com/BrettRey/countability-hpc/tree/master/llm-probe}.
...
The existential probe (\S\ref{sec:existential}) adds a construction dimension. The loose property \term{many} distributes evenly across existential and verb-following frames for both \term{people} and \term{folks} (3.4\% vs.\ 4.0\% existential share), confirming that loose properties are construction-insensitive. \term{Cattle} categorically avoids quantified existentials (zero tokens across all three determinatives) while accepting bare existentials (6 tokens), evidence that quantifiers and existentials jointly amplify individuation demands. The critical cell (\term{three folks} in existentials) is too sparse to test construction-sensitivity directly; an acceptability study (\S\ref{sec:folks}) would probe the gradient the corpus can't reveal.

Together, the corpus data, the descriptive pattern, and the LLM probe provide complementary evidence. The hierarchy captures categorical dissociation (quasi-count nouns), selective suppression of tight properties (\term{folks}), and construction-specific avoidance (quantified existentials with \term{cattle}). The noun $\times$ determinative interaction is confirmed by two independent LLMs; the construction-sensitivity prediction is not, and remains open for human acceptability testing.
...
Traditional accounts treat countability as a lexical feature or small feature bundle: [±count], [±plural], [±bounded]. On this view, \term{cattle} might be specified as [+count, +plural-only] or [+plural, −singular]. This framework established that countability involves multiple morphosyntactic properties that systematically pattern together, a descriptive achievement that any subsequent account must explain. Its strengths are real: formal explicitness, clean integration with broader syntactic theory, and straightforward cross-linguistic parameterization. But the approach can label the classes without deriving the implicational order.
...
Returning to \term{cattle}: if its atoms are inaccessible \citep{grimm2018} or vaguely structured \citep{rothstein2010}, why does \term{many cattle} work while \term{three cattle} fails? The present account's answer (they differ in precision demands) supplements rather than replaces these semantic analyses.
...
The reverse trajectory is visible in \term{data}. Historically the count plural of \term{datum}, it's shifting toward mass status (\term{this data is}, \term{much data}) as the singulative \term{datum} becomes archaic; corpus studies confirm that singular agreement with \term{data} now predominates in most registers \citep{garner2016}. Without a functional singulative, the tight-linkage anchor is lost and the system drifts toward loose-linkage-only status, parallel to the quasi-count pattern but arrived at diachronically. This contrasts with \term{police}, which remains stable because \term{officer} serves the singulative function.
...
\begin{table}[H]
\centering
\caption{Functional anchoring: \term{three} per million tokens of each noun}
\label{tab:anchoring}
\begin{tabular}{@{}l r r l r r r@{}}
\toprule
Quasi-count & Freq. & \term{three}/M & Singulative & Freq. & \term{three}/M & Ratio \\
\midrule
\term{police}  & 220{,}858 & 91  & \term{officers}  & 60{,}161 & 3{,}956 & 44$\times$ \\
\term{poultry} & 4{,}544   & 220 & \term{chickens}  & 8{,}745  & 3{,}431 & 16$\times$ \\
\term{clergy}  & 5{,}879   & 510 & \term{priests}   & 12{,}168 & 2{,}876 & 6$\times$ \\
\bottomrule
\end{tabular}

\smallskip
\footnotesize{Note: \term{Police} count excludes modifier uses. Per-million rates are computed relative to total COCA frequency of each noun form.}
\end{table}

The tighter the suppression in the quasi-count noun, the larger the offset: \term{police}, which most categorically rejects \term{three}, shows the largest ratio (44$\times$). The singulatives don't just exist as alternatives; they do the individuating work that the quasi-count nouns can't.

The diachronic \term{data}/\term{datum} case shows the same logic. As \term{datum} becomes archaic, the singulative anchor is lost and \term{data} drifts toward mass. \term{Police} remains stable because \term{officer} remains in active use.
...
Plural-only nouns like \term{scissors}, \term{trousers}, \term{glasses} (spectacles), and \term{pants} require separate treatment. Unlike \term{police} and \term{cattle}, these nouns denote single objects that happen to have two salient parts. They take plural agreement (\term{these scissors are sharp}) and reject singular forms (\ungram{\term{a scissor}}, \ungram{\term{a trouser}}), but their resistance to singulars reflects morphological fossilization rather than degraded individuation.

The key difference: quasi-count nouns like \term{cattle} have weakened individuation at the semantic level (the noun packages referents as aggregates). Plural-only nouns have normal individuation: a pair of scissors is a single, discrete, enumerable object, but anomalous morphology. Speakers readily enumerate them (\term{three pairs of scissors}, \term{two pairs of trousers}), using the \term{pair of} construction to circumvent the morphological gap. This is unlike \term{cattle}, where \term{three head of cattle} is a specialized counting construction, not a routine singulative.

The distinction matters for the HPC account. Plural-only nouns like \term{scissors} are morphologically defective (no singular form) but fully individuated: they trigger plural agreement, select \term{many}/\term{several} over \term{much}, and resist mass quantification. Quasi-count nouns like \term{cattle} show a different profile: not merely a morphological gap but weakened individuation that bleeds tight-linkage properties even where morphology permits them. If the quasi-count pattern were purely morphological, plural-only nouns should show the same ordered dissociation; they don't.

A full treatment would need to address why English developed these forms, whether the \term{pair of} construction is itself a loose- or tight-linkage property, and how plural-only nouns behave cross-linguistically. I set these questions aside here.
...
The hierarchy rests on the notion that count properties differ in the \textit{precision} of individuation they require. I've characterized this informally: \term{three} demands exact enumeration; \term{many} tolerates approximate magnitude. A natural objection is that this is circular: we know \term{three} requires more precision than \term{many} because \term{three cattle} fails and \term{many cattle} succeeds, but that's the explanandum, not the explanans. The objection deserves a careful answer, and the answer turns on field-relative projectibility.
...
Pointwise mutual information (PMI) scores from COCA provide independent confirmation. For quasi-count nouns, tight determinatives show negative PMI (the combination is rarer than chance would predict given the individual frequencies of determinative and noun), while loose determinatives show positive PMI for the same nouns. Table~\ref{tab:precision} extends this comparison across eight determinatives that dissociate frequency from precision. The critical case is \term{two} vs.\ \term{numerous}: \term{two} is 32 times more frequent than \term{numerous}, yet \term{two cattle} is repelled ($\text{PMI} = -2.46$) while \term{numerous cattle} is attracted ($+0.97$). Every tight determinative repels quasi-count nouns regardless of its frequency; every loose determinative attracts them regardless of its rarity. The ordering tracks semantic precision, not collocation frequency.

Semantic accounts offer candidate explanations for \textit{why} the ordering has the shape it does. \textcite{grimm2018} analyzes collectives and aggregates in terms of atomic accessibility: atoms exist in the denotation but aren't accessible to certain quantificational operations. \textcite{rothstein2010} distinguishes counting (requires context-relative atomicity, where atoms must be stable under context shift) from measuring (requires only a monotone scale). On this analysis, \term{three cattle} fails because \term{three} counts and \term{cattle}'s atoms aren't accessible; \term{many cattle} succeeds because \term{many} measures. \textcite{chierchia1998}'s mass/count parameter turns on whether atoms are encoded in lexical semantics; quasi-count nouns would have atoms semantically present but with degraded accessibility. \textcite{hackl2000} treats \term{many} as a context-dependent degree modifier, which connects it to vagueness theory rather than counting.

These are analyses from formal semantics, projecting relative to truth conditions and compositionality. A semanticist might conclude that \term{many} and \term{much} are the same operation (degree modification over a scale) and that \term{many cattle} tells us nothing about \term{cattle}'s individuation. But morphosyntactically, \term{many} and \term{much} are in complementary distribution: \term{many cattle} yes, \term{much cattle} no, \term{many furniture} no, \term{much furniture} yes. Whatever \term{many} does semantically, it selects for something that mass nouns lack and quasi-count nouns retain. The morphosyntactic hierarchy captures this distributional fact; the semantic frameworks explain why the fact obtains. Both levels are real; they answer different questions. This is field-relative projectibility (\S\ref{sec:levels}): the same data yield different right-sized categories for different inferential purposes.
...
The \term{folks} construction-sensitivity prediction is tested here only with a preregistered LLM judgment probe, not with a controlled speaker-judgment experiment. The LLM probe should be read as a supplementary test of corpus-based extrapolations, not as establishing community repertoire on its own. The COCA frequency data reported in \S\ref{sec:corpus} support the predicted asymmetry for the hierarchy as a whole, but the specific existential--agentive interaction awaits human acceptability data.
...
If grammatical categories are homeostatic clusters maintained by inference, then every grammatical choice involves negotiation. The warmth of \term{folks} costs precision; the stability of \term{cattle} requires functional anchoring. Grammar isn't a system of arbitrary rules but a record of countless speaker decisions about what to foreground and what to trade away.

If this is right, then the borders of grammar aren't walls but gradients~-- maintained, negotiated, and occasionally redrawn by every speaker who uses them. The student who writes \ungram{\term{three furnitures}}, the editor who debates \term{data}, and the Texan who says \term{folks} are testing the boundaries of a homeostatic system. The HPC view predicts such variation: when speakers operate at cluster boundaries, some choices reinforce the equilibrium and others test it. Some tests fail; others, over time, become the new equilibrium. \term{Pea} was once outside the cluster. So, perhaps, was \term{cattle} as a quasi-count noun. The cluster's stability is real, but it's a dynamic stability~-- held in place by millions of small inferences, every day, in every conversation.
...
\midrule
\term{people} & \term{three}   & existential & 7.00 & 0.00 & 6.91 & 0.28 \\
              &                & agentive    & 7.00 & 0.00 & 6.94 & 0.24 \\
              & \term{several} & existential & 7.00 & 0.00 & 6.91 & 0.28 \\
              &                & agentive    & 7.00 & 0.00 & 6.94 & 0.24 \\
\midrule
\term{folks}  & \term{three}   & existential & 4.95 & 0.22 & 5.92 & 0.27 \\
              &                & agentive    & 4.95 & 0.22 & 5.94 & 0.24 \\
              & \term{several} & existential & 6.00 & 0.00 & 5.92 & 0.27 \\
...
% X labels
\node[font=\small, below] at (1.5,-0.3) {\term{three}};
\node[font=\small, below] at (4.0,-0.3) {\term{several}};
...
% Legend
\node[fill=houseprimary, circle, inner sep=2pt] at (10,4.5) {};
\node[font=\small, anchor=west] at (10.2,4.5) {\term{people}};
\node[fill=housesecondary, circle, inner sep=2pt] at (10,4.0) {};
\node[font=\small, anchor=west] at (10.2,4.0) {\term{folks}};

\end{tikzpicture}
\caption{LLM probe cell means. Parallel lines within each determinative panel confirm no existential--agentive interaction. Vertical separation between \term{people} (blue) and \term{folks} (coral) confirms the hierarchy. Claude distinguishes \term{three folks} (4.95) from \term{several folks} (6.00); GPT-5.4 does not.}
\label{fig:probe}
\end{figure}

\textbf{Hierarchy confirmed.} Both models rated \term{three folks} lower than \term{three people} (Claude: 4.95 vs.\ 7.00; GPT-5.4: 5.93 vs.\ 6.92). The tight property is suppressed for the boundary noun, matching the COCA corpus pattern. Claude additionally distinguished tight from moderate: \term{several folks} (6.00) was rated higher than \term{three folks} (4.95). GPT-5.4 did not make this distinction (\term{three folks} $\approx$ \term{several folks} $\approx$ 5.93), suggesting it represents the \term{folks} degradation as uniform rather than precision-graded.

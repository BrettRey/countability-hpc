...
\begin{tikzpicture}[
    axis/.style={thick},
    grid/.style={gray!20},
    bar/.style={fill=green!60, draw=gray!50, line width=4pt, rounded corners=1pt},
...
\centering
\begin{tikzpicture}[
    bar/.style={fill=green!60, draw=gray!50},
    label/.style={font=\small}
]
...
\begin{tikzpicture}[
    label/.style={font=\small},
    dot three/.style={fill=red!70, draw=red!70, circle, inner sep=1.5pt},
    dot many/.style={fill=blue!60, draw=blue!60, circle, inner sep=1.5pt},
    ci three/.style={red!70, thick},
    ci many/.style={blue!60, thick}
]
...
\node[font=\scriptsize, red!70] at (-0.15,0.47) {0};
...
\end{tikzpicture}
\caption{Per-million frequency of \term{three} N (red) and \term{many} N (blue) with 95\% Poisson confidence intervals, plotted on a $\log_{10}$ scale. Singulatives cluster at top right (both quantifiers well attested). Quasi-count nouns show the dissociation: \term{many} rates overlap with singulatives, but \term{three} rates plummet. Wide intervals for small counts (\term{three clergy}, \term{many poultry}, \term{many vermin}) indicate that the point estimates are unreliable; the directional pattern is not. Head uses only for \term{police} and \term{cattle}; \term{many} excludes \term{how many}.}
\label{fig:dotplot}
\end{figure}

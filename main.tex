...
Feature-bundle accounts (e.g., \citealt{chomsky1965}; see \citealt{allan1980} for critical review) encode the clustering but can't explain why it persists. Prototype descriptions (e.g., \citealt{taylor2003,aarts2007}) capture gradience but not the ordering. A homeostatic property cluster (HPC) view does better: countability is a cluster maintained by causal mechanisms rather than mere resemblance. The puzzle is why the cluster is so resistant to random drift but still allows orderly partial dissociation.
...
The present account's notion of \enquote{precision} maps onto these frameworks as follows: tight properties require accessible, countable atoms \citep{grimm2018}, exact cardinality \citep{rothstein2010}, or encoding in lexical semantics \citep{chierchia1998}. Loose properties tolerate inaccessible atoms, measure-based quantification, or pragmatic construal. The hierarchy is thus derivable from standard semantic frameworks if we rank count properties by their atomic-accessibility demands. An explicit account of the cluster structure and the mechanisms maintaining it would specify these demands as semantic presuppositions or selectional restrictions, deriving the ordering from independently motivated semantic primitives. The HPC view predicts that no classical formalization (necessary and sufficient conditions) is available; what it offers instead (and what linguistic categories require) is an account of the mechanisms that maintain the cluster despite fuzzy boundaries and graded membership. I leave this for future work, noting only that the informal characterization is sufficient for the empirical predictions developed here.

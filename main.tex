...
The robustness of loose-linkage properties is partly semantic (their content doesn't encode sharp atomic boundaries) and partly inferential (the system imposes a lower precision threshold on them).
...
This hierarchy explains the quasi-count pattern documented in descriptive grammars and predicts where new or shifting nouns should fall based on their individuation profile.
...
On the present account, this is exactly what we should expect for a noun whose individuation profile is weakening but hasn't fully collapsed. Morphologically, \term{folks} is simply a plural; it lacks a productive singular (\ungram{\term{a folks}}) just as the basic collective sense of \term{folk} lacks one (\ungram{\term{a folk}} in the ordinary 'people' sense). The difference from \term{cattle} and \term{police} is semantic and pragmatic. Uses like \term{you folks}, \term{folks around here}, and \term{our folks back home} package the referents as in-group collectives with a strong solidarity flavour. The individual atoms are still there ontically, but the noun's dominant sense foregrounds the group, backgrounding precise, enumerable units.
...
Historical shifts provide evidence for the mechanism. English \term{pea} arose by back-formation from \term{pease}, which was historically non-count (cf.\ \term{pease porridge}). The final /z/ was reanalyzed as plural marking, and a singular \term{pea} was created to complete the count cluster. If the historical record shows that looser properties (plural agreement, \term{many peas}) were established before tighter ones (\term{a pea}, \term{three peas}), that would confirm the predicted acquisition order.
...
Existing work on the acquisition of mass/count syntax \citep{barner2005} provides some relevant data, but the specific predictions about ordered acquisition and overgeneralisation for quasi-count nouns haven't been tested. Corpus studies of child-directed speech and longitudinal production data would be informative.
...
Either approach would need to derive the hierarchy, not just describe it. The ordering of tight, moderate, and loose properties should fall out from the formalisation, with properties ranked by their precision demands. I leave this formalisation for future work, noting only that the informal characterisation is sufficient for the empirical predictions developed here.
